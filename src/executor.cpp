...
{
    executor::executor(std::shared_ptr<ratio::solver> slv, const utils::rational &units_per_tick) noexcept : slv(slv), exec_theory(slv->get_sat().new_theory<executor_theory>(*this)), units_per_tick(units_per_tick) { exec_theory.init(); }
